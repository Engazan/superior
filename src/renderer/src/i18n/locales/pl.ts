import type { MessageKey } from './en'

export const pl: Record<MessageKey, string> = {
  'sidebar.openProject': 'Otwórz / sklonuj projekt',
  'sidebar.noWorkspaces': 'Brak folderów — otwórz jeden.',
  'sidebar.removeFromList': 'Usuń z listy',
  'sidebar.addWorkspace': 'Dodaj workspace',
  'sidebar.newWorkspacePlaceholder': 'Nazwa workspace…',
  'sidebar.renameWorkspace': 'Kliknij dwukrotnie, aby zmienić nazwę',
  'sidebar.renameWorkspaceAction': 'Zmień nazwę workspace',
  'sidebar.removeWorkspace': 'Usuń workspace',
  'sidebar.removeWorkspaceConfirm': 'Usunąć workspace „{name}”? Uruchomione terminale zostaną zamknięte.',
  'sidebar.removeFolder': 'Usuń folder',
  'sidebar.removeFolderConfirm':
    'Usunąć „{name}” z listy? Jego workspaces zostaną zamknięte, a działające terminale zatrzymane. Pliki na dysku pozostaną nietknięte.',
  'menu.folderActions': 'Akcje folderu',
  'menu.workspaceActions': 'Akcje workspace',
  'grid.dividerHint': 'Przeciągnij, aby zmienić rozmiar · z Alt swobodnie',
  'profile.label': 'Profil',
  'profile.switch': 'Przełącz profil',
  'profile.manage': 'Zarządzaj profilami…',
  'profile.manageTitle': 'Zarządzanie profilami',
  'profile.manageDescription':
    'Profile przechowują oddzielne zestawy folderów. Przełączaj się między nimi z paska narzędzi.',
  'profile.name': 'Nazwa profilu',
  'profile.active': 'Aktywny',
  'profile.color': 'Kolor',
  'profile.add': 'Dodaj profil',
  'profile.addPlaceholder': 'Nazwa nowego profilu…',
  'profile.delete': 'Usuń profil',
  'profile.deleteConfirm': 'Usunąć „{name}” i wszystkie jego foldery? Tej operacji nie można cofnąć.',
  'profile.deleteTitle': 'Usuń profil',
  'confirm.integrationRemoveTitle': 'Usuń integrację',
  'confirm.integrationRemoveMessage': 'Usunąć „{name}”? Jej token dostępu zostanie skasowany.',
  'confirm.daemonKillTitle': 'Zakończ sesję',
  'confirm.daemonKillMessage': 'Zakończyć „{label}”? Działający proces zostanie zatrzymany.',
  'confirm.daemonKillAllTitle': 'Zakończ wszystkie sesje',
  'confirm.daemonKillAllMessage': 'Zakończyć wszystkie sesje ({n})? Wszystkie działające procesy zostaną zatrzymane.',
  'toast.profileDeleted': 'Profil „{name}” usunięty',
  'toast.presetSaved': 'Preset „{name}” zapisany',
  'toast.presetDeleted': 'Preset „{name}” usunięty',
  'toast.integrationSaved': 'Integracja „{name}” zapisana',
  'toast.integrationRemoved': 'Integracja „{name}” usunięta',
  'toast.daemonKilled': '„{label}” zakończony',
  'toast.daemonsKilled': 'Zakończone sesje: {n}',
  'toast.layoutSaved': 'Preset układu „{name}” zapisany',
  'folder.edit': 'Edytuj',
  'folder.editModalTitle': 'Edytuj folder',
  'folder.editModalDescription': 'Dostosuj sposób wyświetlania tego folderu. Jego ścieżka pozostaje bez zmian.',
  'folder.displayName': 'Wyświetlana nazwa',
  'folder.displayNameHint': 'Pozostaw puste, aby użyć nazwy folderu.',
  'folder.icon': 'Ikona',
  'folder.color': 'Kolor tła w panelu bocznym',
  'folder.uploadIcon': 'Prześlij ikonę',
  'folder.removeIcon': 'Usuń',
  'folder.path': 'Ścieżka',
  'folder.saveAction': 'Zapisz zmiany',
  'sidebar.reorderFolder': 'Przeciągnij, aby zmienić kolejność',
  'sidebar.runningTerminals': 'Działające terminale',
  'sidebar.diffStat': 'Niezatwierdzone zmiany (dodane / usunięte linie)',
  'sidebar.workingTerminals': 'Pracuje…',
  'update.available': 'Dostępna wersja {version}',
  'update.action': 'Aktualizuj',
  'update.downloading': 'Pobieranie… {percent}%',
  'update.restart': 'Uruchom ponownie i zainstaluj',
  'update.failed': 'Aktualizacja nie powiodła się — pobierz ręcznie',
  'update.openPage': 'Otwórz stronę pobierania',
  'sidebar.settings': 'Ustawienia',
  'common.toggleSidebar': 'Przełącz pasek boczny',
  'common.toggleRightSidebar': 'Przełącz prawy panel',
  'rightPanel.files': 'Pliki',
  'rightPanel.changes': 'Zmiany',
  'changes.loading': 'Ładowanie zmian…',
  'changes.notRepository': 'To nie jest repozytorium Git.',
  'changes.empty': 'Brak zmian',
  'changes.binary': 'Plik binarny',
  'changes.tooLarge': 'Plik zbyt duży do wyświetlenia',
  'changes.truncated': 'Wyświetlono tylko pierwsze 500 nieśledzonych plików.',
  'changes.refresh': 'Odśwież',
  'changes.staged': 'Staged',
  'changes.unstaged': 'Zmiany',
  'changes.stage': 'Dodaj do staged',
  'changes.unstage': 'Usuń ze staged',
  'changes.stageAll': 'Dodaj wszystko',
  'changes.unstageAll': 'Usuń wszystko',
  'changes.commit': 'Commituj',
  'changes.commitPlaceholder': 'Wiadomość commita…',
  'changes.committed': 'Zmiany scommitowane',
  'changes.push': 'Push',
  'changes.pull': 'Pull',
  'changes.publish': 'Opublikuj',
  'changes.pushed': 'Wypchnięto',
  'changes.pulled': 'Pobrano',
  'rightPanel.history': 'Historia',
  'history.loading': 'Wczytywanie historii…',
  'history.empty': 'Brak commitów.',
  'files.loading': 'Ładowanie plików…',
  'files.empty': 'Pusty folder',
  'files.noFolder': 'Nie wybrano folderu',
  'files.search': 'Szukaj plików…',
  'files.searching': 'Szukam…',
  'files.noResults': 'Brak pasujących plików',
  'files.searchTruncated': 'Pokazano tylko pierwsze wyniki',
  'contentSearch.placeholder': 'Szukaj tekstu w plikach…',
  'contentSearch.noResults': 'Brak pasującej treści',
  'contentSearch.truncated': 'Pokazano tylko pierwsze dopasowania treści',
  'preview.loading': 'Ładowanie podglądu…',
  'preview.rendered': 'Podgląd',
  'preview.raw': 'Źródło',
  'preview.notAvailable': 'Podgląd niedostępny',
  'preview.openRaw': 'Otwórz plik',
  'preview.copyPath': 'Kopiuj ścieżkę',
  'preview.copied': 'Skopiowano',
  'preview.pdfDisabled': 'Podgląd PDF jest wyłączony. Otwórz go w domyślnej aplikacji.',
  'preview.binary': 'Tego typu pliku nie można wyświetlić.',
  'preview.tooLargeImage': 'Obraz jest zbyt duży do podglądu.',
  'preview.truncated': 'Plik jest duży — pokazano tylko pierwszą część.',
  'preview.unsaved': 'Niezapisane zmiany',
  'preview.saveFailed': 'Zapis nie powiódł się',
  'common.cancel': 'Anuluj',
  'common.save': 'Zapisz',
  'common.delete': 'Usuń',
  'common.edit': 'Edytuj',
  'settings.back': 'Wstecz',
  'settings.personal': 'Osobiste',
  'settings.workflow': 'Przepływ pracy',
  'settings.keyboard': 'Klawiatura',
  'settings.shellCommand': 'Wiersz poleceń',
  'keyboard.desc':
    'Dostosuj skróty klawiszowe. Kliknij skrót i naciśnij nową kombinację klawiszy.',
  'keyboard.colAction': 'Akcja',
  'keyboard.colShortcut': 'Skrót',
  'keyboard.toggleSidebar': 'Przełącz panel boczny',
  'keyboard.openSettings': 'Otwórz ustawienia',
  'keyboard.maximizeCell': 'Maksymalizuj aktywny terminal',
  'keyboard.openLauncher': 'Dodaj terminal (wybór presetu)',
  'keyboard.toggleRightPanel': 'Przełącz panel Pliki / Zmiany',
  'keyboard.closeFocusedCell': 'Zamknij aktywny terminal',
  'keyboard.closePreview': 'Zamknij podgląd pliku',
  'keyboard.saveFile': 'Zapisz plik',
  'keyboard.prevTerminal': 'Poprzedni terminal',
  'keyboard.nextTerminal': 'Następny terminal',
  'keyboard.openFolder': 'Otwórz folder',
  'keyboard.prevWorkspace': 'Poprzedni workspace',
  'keyboard.nextWorkspace': 'Następny workspace',
  'keyboard.prevProfile': 'Poprzedni profil',
  'keyboard.nextProfile': 'Następny profil',
  'keyboard.manageProfiles': 'Zarządzaj profilami',
  'keyboard.searchTerminal': 'Szukaj w terminalu',
  'keyboard.searchFileContents': 'Szukaj tekstu w plikach projektu',
  'keyboard.openPalette': 'Paleta poleceń',
  'keyboard.globalHotkey': 'Globalny skrót pokaż/ukryj',
  'keyboard.globalHotkeyDesc': 'Systemowy skrót przywołujący Superior na wierzch (lub ukrywający go) z dowolnej aplikacji.',
  'keyboard.globalHotkeyClear': 'Wyłącz',
  'keyboard.globalHotkeyUnset': 'Nie ustawiono',
  'palette.placeholder': 'Wpisz polecenie…',
  'palette.noResults': 'Brak pasujących poleceń',
  'palette.sectionWorkspaces': 'Workspaces',
  'palette.sectionProfiles': 'Profile',
  'palette.sectionTerminals': 'Terminale',
  'palette.sectionView': 'Widok',
  'palette.sectionGit': 'Git',
  'palette.sectionSettings': 'Ustawienia',
  'search.placeholder': 'Szukaj w terminalu…',
  'search.next': 'Następne dopasowanie',
  'search.previous': 'Poprzednie dopasowanie',
  'search.close': 'Zamknij wyszukiwanie',
  'notify.setting': 'Powiadomienia',
  'notify.settingDesc': 'Powiadom, gdy terminal wymaga uwagi lub jego proces zakończy się, a aplikacja jest w tle.',
  'notify.finishedTitle': '{label} wymaga uwagi',
  'notify.finishedBody': 'Sprawdź terminal w workspace {workspace}.',
  'keyboard.recording': 'Naciśnij klawisze…',
  'keyboard.reset': 'Przywróć domyślne',
  'settings.appearance': 'Wygląd',
  'settings.terminalPresets': 'Presety terminala',
  'settings.daemons': 'Demony',
  'settings.prompts': 'Prompty',
  'prompts.desc': 'Zapisane prompty wstawiane do aktywnego terminala jednym kliknięciem.',
  'prompts.add': 'Dodaj prompt',
  'prompts.empty': 'Brak promptów.',
  'prompts.name': 'Nazwa',
  'prompts.namePlaceholder': 'np. Review checklist',
  'prompts.text': 'Tekst promptu',
  'prompts.textPlaceholder': 'Tekst wstawiany do terminala…',
  'prompts.addTitle': 'Dodaj prompt',
  'prompts.editTitle': 'Edytuj prompt',
  'prompts.deleteTitle': 'Usuń prompt',
  'prompts.deleteConfirm': 'Usunąć „{name}”?',
  'prompts.deleted': 'Prompt „{name}” usunięty',
  'prompts.saved': 'Prompt „{name}” zapisany',
  'prompts.insert': 'Wstaw prompt',
  'prompts.pickerPlaceholder': 'Szukaj promptów…',
  'prompts.pickerHint': 'Enter wstawia · Shift+Enter wstawia i wysyła',
  'rightPanel.tasks': 'Zadania',
  'tasks.title': 'Kolejka zadań',
  'tasks.noFolder': 'Nie wybrano projektu.',
  'tasks.empty': 'Brak zadań — dodaj jedno poniżej.',
  'tasks.promptPlaceholder': 'Co ma zrobić agent?…',
  'tasks.worktree': 'Nowa gałąź',
  'tasks.worktreeHint':
    'Uruchamia zadanie w nowym izolowanym branch workspace (git worktree) zamiast w bieżącym.',
  'tasks.add': 'Dodaj do kolejki',
  'tasks.hint':
    'Uruchamia „polecenie presetu + prompt”. Następne zadanie startuje, gdy terminal poprzedniego się zakończy — zamknij CLI albo użyj presetu headless (np. claude -p), aby kolejka działała w pełni automatycznie.',
  'tasks.pause': 'Wstrzymaj',
  'tasks.resume': 'Wznów',
  'tasks.paused': 'Kolejka wstrzymana',
  'tasks.clearFinished': 'Wyczyść ukończone',
  'tasks.cancel': 'Anuluj zadanie',
  'tasks.delete': 'Usuń zadanie',
  'tasks.goTo': 'Przejdź do workspace',
  'tasks.statusRunning': 'Działa',
  'tasks.statusQueued': 'W kolejce',
  'tasks.statusDone': 'Gotowe',
  'tasks.statusFailed': 'Niepowodzenie',
  'tasks.statusCanceled': 'Anulowane',
  'tasks.finishedGroup': 'Ukończone',
  'tasks.exitCode': 'kod wyjścia {code}',
  'tasks.errPresetGone': 'Preset zadania już nie istnieje.',
  'tasks.errNoWorkspace': 'Ten folder nie ma workspace, w którym można uruchomić zadanie.',
  'broadcast.toggle': 'Broadcast do wszystkich terminali',
  'broadcast.placeholder': 'Wyślij do {n} terminali…',
  'broadcast.exit': 'Zakończ broadcast',
  'broadcast.target': 'Przełącz ten terminal jako cel broadcastu',
  'settings.language': 'Język',
  'daemons.desc':
    'Aktywne procesy terminala należące do demona w tle. Działają dalej, gdy aplikacja jest zamknięta.',
  'daemons.empty': 'Brak działających sesji demona.',
  'daemons.kill': 'Zakończ',
  'daemons.killAll': 'Zakończ wszystkie',
  'daemons.refresh': 'Odśwież',
  'daemons.colCommand': 'Polecenie',
  'daemons.colWorkspace': 'Workspace',
  'daemons.colPid': 'PID',
  'daemons.orphan': 'Bez workspace',
  'appearance.desc': 'Motyw, język i sposób sygnalizowania aktywności przez aplikację.',
  'appearance.theme': 'Motyw',
  'appearance.themeDesc':
    'Wybierz wygląd aplikacji. System podąża za ustawieniem systemu operacyjnego.',
  'appearance.sidebarWorkspaceTools': 'Narzędzia workspace w panelu bocznym',
  'appearance.sidebarWorkspaceToolsDesc':
    'Pokaż wyszukiwanie, filtr ulubionych i ostatnio odwiedzane workspace w panelu bocznym.',
  'appearance.attentionColor': 'Kolor powiadomienia',
  'appearance.attentionColorDesc':
    'Kolor, którym pulsuje karta przestrzeni roboczej, gdy jeden z jej terminali skończy pracę, a Ciebie nie ma.',
  'appearance.resetColor': 'Resetuj',
  'theme.light': 'Jasny',
  'theme.dark': 'Ciemny',
  'theme.system': 'Systemowy',
  'theme.transparent': 'Przezroczysty',
  'theme.gradient': 'Gradient',
  'theme.gradientLight': 'Jasny gradient',
  'language.desc': 'Wybierz język interfejsu.',
  'presets.add': 'Dodaj preset',
  'presets.colIcon': 'Ikona',
  'presets.colName': 'Nazwa',
  'presets.colDescription': 'Opis',
  'presets.colCommand': 'Polecenie',
  'presets.colActive': 'Aktywny',
  'presets.colActions': 'Akcje',
  'presets.empty': 'Brak presetów — dodaj jeden.',
  'presets.dragReorder': 'Przeciągnij, aby zmienić kolejność',
  'presets.deleteTitle': 'Usuń preset',
  'presets.deleteConfirm': 'Usunąć „{name}“? Tej operacji nie można cofnąć.',
  'memory.title': 'Niestandardowe presety pamięci terminala',
  'memory.description': 'Profile Claude i Codex znalezione w oddzielnych folderach konfiguracji.',
  'memory.add': 'Dodaj własną pamięć',
  'memory.loading': 'Ładowanie własnych pamięci…',
  'memory.empty': 'Nie znaleziono własnych pamięci Claude ani Codex.',
  'memory.aliasExists': 'Alias w {files}',
  'memory.addAlias': 'Dodaj alias',
  'memory.inTerminalPresets': 'W presetach terminala',
  'memory.addToTerminalPresets': 'Dodaj do presetów terminala',
  'memory.createTitle': 'Dodaj własną pamięć',
  'memory.provider': 'Dostawca',
  'memory.name': 'Nazwa',
  'memory.nameHint': 'Tworzy ~/{directory} i alias {alias}.',
  'memory.creating': 'Tworzenie…',
  'memory.create': 'Utwórz',
  'cli.title': 'Dostępność CLI',
  'cli.description':
    'Sprawdza, czy te narzędzia CLI są zainstalowane i uruchamialne w terminalu, który otwiera ta aplikacja. CLI może być zainstalowane, lecz niewidoczne dla aplikacji, jeśli jego PATH ustawiono tylko w interaktywnym pliku powłoki (np. ~/.zshrc).',
  'cli.checking': 'Sprawdzanie…',
  'cli.available': 'Dostępne',
  'cli.notOnPath': 'Zainstalowane, brak w PATH',
  'cli.notFound': 'Niezainstalowane',
  'cli.fix': 'Napraw automatycznie',
  'cli.fixing': 'Naprawianie…',
  'cli.fixed': 'Dodano do {file}. Otwórz nowy terminal, aby użyć.',
  'cli.fixFailed': 'Nie udało się naprawić automatycznie. Sprawdź instalację ręcznie.',
  'cli.recheck': 'Sprawdź ponownie',
  'shell.status': 'Status',
  'shell.title': 'Otwórz folder z terminala',
  'shell.description':
    'Instaluje polecenie `superior`, które otwiera dowolny folder w aplikacji bezpośrednio z terminala. Jeśli aplikacja już działa, folder otworzy się w bieżącym oknie.',
  'shell.example': 'Uruchom w folderze, który chcesz otworzyć w aplikacji Superior.',
  'shell.install': 'Zainstaluj polecenie',
  'shell.reinstall': 'Zainstaluj ponownie',
  'shell.installing': 'Instalowanie…',
  'shell.available': 'Zainstalowane',
  'shell.notOnPath': 'Zainstalowane, brak w PATH',
  'shell.notInstalled': 'Niezainstalowane',
  'shell.installed': 'Zainstalowane. Wypróbuj `superior .` w dowolnym folderze.',
  'shell.installedReopen': 'Zainstalowane i dodane do {file}. Otwórz nowy terminal i wypróbuj `superior .`.',
  'shell.installFailed': 'Instalacja nie powiodła się: {message}',
  'form.addTitle': 'Dodaj preset',
  'form.editTitle': 'Edytuj preset',
  'form.icon': 'Ikona',
  'form.customImage': 'Własny obraz…',
  'form.customImageSelected': 'Wybrano własny obraz',
  'form.name': 'Nazwa',
  'form.nickname': 'Pseudonim (opcjonalnie)',
  'form.nicknamePlaceholder': 'np. backend',
  'form.description': 'Opis',
  'form.command': 'Polecenie',
  'form.color': 'Kolor górnego paska',
  'form.colorNone': 'Brak',
  'form.colorCustom': 'Własny…',
  'launchers.terminalPresets': 'Presety terminala',
  'launchers.openWorkspaceFirst': 'Najpierw otwórz obszar roboczy',
  'launchers.run': 'Uruchom: {command}',
  'launcher.workingFolder': 'Folder roboczy',
  'launcher.howMany': 'Ile terminali?',
  'launcher.terminalN': 'Terminal {n}',
  'launcher.terminalCount': 'Terminale: {n}',
  'launcher.nicknamePlaceholder': 'pseudonim',
  'launcher.start': 'Uruchom',
  'launcher.noPresets': 'Brak presetów — dodaj jeden w ustawieniach.',
  'launcher.tabPreset': 'Preset',
  'launcher.tabCustom': 'Nowy',
  'launcher.savedPresets': 'Zapisane presety',
  'launcher.newPreset': '+ Nowy',
  'launcher.noLayouts': 'Brak zapisanych presetów — utwórz przez + Nowy.',
  'launcher.deletePreset': 'Usuń preset',
  'launcher.presetNamePlaceholder': 'Nazwa presetu',
  'launcher.untitledPreset': 'Bez nazwy',
  'launcher.saveAndStart': 'Zapisz i uruchom',
  'launcher.launchOnOpen': 'Uruchom przy otwarciu tego workspace',
  'launcher.cancel': 'Anuluj',
  'tab.defaultName': 'Karta {n}',
  'tab.new': 'Nowa karta',
  'tab.rename': 'Kliknij dwukrotnie, aby zmienić nazwę',
  'tab.renameAction': 'Zmień nazwę karty',
  'tab.close': 'Zamknij kartę',
  'terminal.addTerminal': 'Dodaj terminal',
  'terminal.managePresets': 'Zarządzaj presetami…',
  'terminal.empty': 'Żaden agent nie działa. Wybierz obszar roboczy i uruchom agenta.',
  'terminal.noWorkspace': 'Wybierz lub dodaj workspace.',
  'terminal.stopClose': 'Zatrzymaj i zamknij',
  'terminal.closeSession': 'Zamknij sesję',
  'terminal.maximize': 'Maksymalizuj',
  'terminal.restore': 'Przywróć',
  'terminal.restart': 'Uruchom polecenie ponownie',
  'terminal.restoreAll': 'Przywróć wszystkie terminale',
  'terminal.interruptedBanner':
    'Terminale przerwane przez wyłączenie systemu: {count}. Ich polecenia i układ zostały zapisane.',
  'terminal.setNickname': 'Ustaw pseudonim',
  'terminal.focusHint': 'Fokus',
  'terminal.nicknamePlaceholder': 'Pseudonim',
  'terminal.restartHint': 'naciśnij Enter, aby uruchomić ponownie',
  'terminal.imagePasted': 'Obraz dołączony do terminala',
  'terminal.imagePasteFailed': 'Nie udało się wkleić obrazu',
  'terminal.copyFailed': 'Nie udało się skopiować zaznaczonego tekstu',
  'terminal.copy': 'Kopiuj',
  'usage.context': 'Kontekst',
  'usage.total': 'Razem',
  'usage.tokens': 'tokenów',
  'usage.input': 'Wejście',
  'usage.output': 'Wyjście',
  'usage.cacheRead': 'Odczyt cache',
  'usage.cacheWrite': 'Zapis cache',
  'usage.cost': 'Koszt (szac.)',
  'usage.fiveHour': '5h limit',
  'usage.fiveHourShort': '5h',
  'usage.sevenDayShort': '7d',
  'usage.contextShort': 'ctx',
  'usage.primary': 'Wartość na pasku',
  'usage.primaryDesc': 'Która wartość ma być widoczna na pasku. Najedź, aby zobaczyć wszystkie.',
  'usage.primaryRemaining': 'Pozostało 5h',
  'usage.primarySevenDay': 'Pozostało 7d',
  'usage.primaryCost': 'Koszt',
  'usage.primaryTokens': 'Tokeny',
  'usage.primaryContext': 'Kontekst',
  'usage.sevenDay': '7d limit',
  'usage.remaining': 'pozostało',
  'usage.used': 'wykorzystano',
  'footer.title': 'Zużycie kont',
  'footer.options': 'Profile i opcje',
  'footer.refresh': 'Odśwież',
  'footer.close': 'Zamknij',
  'footer.loading': 'Ładowanie zużycia…',
  'footer.unavailable': 'Zużycie niedostępne',
  'footer.notSignedIn': 'Nie znaleziono dostępnego logowania w tym profilu',
  'footer.expired': 'Zaloguj się ponownie w tym profilu',
  'footer.rateLimited': 'Zbyt wiele żądań — ponowienie później',
  'footer.noData': 'Nie otrzymano limitów subskrypcji',
  'footer.remaining': 'Pokaż pozostałe %',
  'footer.compact': 'Kompaktowe szczegóły',
  'footer.show': 'Pokaż w stopce:',
  'footer.updated': 'Sprawdzono',
  'footer.sharedLimit': 'Profile zalogowane na to samo konto współdzielą jego limity. Limity nie są sumowane.',
  'footer.manage': 'Zarządzaj presetami custom memory',
  'usage.resets': 'reset o',
  'usage.tracking': 'Zużycie Claude w pasku',
  'usage.trackingDesc':
    'Pokazuje pozostały limit 5h/7d i koszt dla terminali Claude. Instaluje pomocnika status line w folderach konfiguracyjnych Claude uruchamianych przez aplikację; istniejący status line nadal działa.',
  'window.minimize': 'Minimalizuj',
  'window.maximize': 'Maksymalizuj',
  'window.restore': 'Przywróć',
  'window.close': 'Zamknij',
  'error.noWorkspace': 'Nie wybrano obszaru roboczego. Najpierw dodaj lub wybierz folder.',
  'error.notARepo': 'Ten folder nie jest repozytorium Git.',
  'error.branchExists': 'Gałąź o tej nazwie już istnieje.',
  'error.branchCheckedOut': 'Ta gałąź jest już wyewidencjonowana w innym worktree.',
  'error.worktreeCreateFailed': 'Nie udało się utworzyć worktree: {message}',
  'sidebar.addWorktreeWorkspace': 'Nowy workspace z gałęzią…',
  'sidebar.worktreeBranchPlaceholder': 'Nazwa gałęzi…',
  'sidebar.createNewBranch': 'Nowa gałąź',
  'sidebar.useExistingBranch': 'Istniejąca',
  'sidebar.worktreeBadge': 'Workspace z gałęzią (izolowany worktree)',
  'workspace.createModalTitle': 'Utwórz workspace',
  'workspace.createModalDescription':
    'Wybierz, jak ten workspace ma współdzielić pliki projektu i stan Git.',
  'workspace.type': 'Typ workspace',
  'workspace.standardType': 'Współdzielony workspace',
  'workspace.standardTypeDescription': 'Własne terminale i układ, współdzielone pliki projektu.',
  'workspace.branchType': 'Workspace z gałęzią',
  'workspace.branchTypeDescription': 'Izolowana gałąź Git i pliki robocze.',
  'workspace.createTitle': 'Dodaj workspace',
  'workspace.createDescription':
    'Utwórz osobny kontekst terminali i układu dla tego folderu projektu.',
  'workspace.name': 'Nazwa workspace',
  'workspace.namePlaceholder': 'np. Debugowanie API',
  'workspace.nameHint': 'Użyj krótkiej nazwy opisującej zadanie lub kontekst.',
  'workspace.duplicateName': 'Workspace o tej nazwie już istnieje w tym folderze.',
  'workspace.sharedFolder': 'Współdzielony folder',
  'workspace.sharedFolderDescription':
    'Ten workspace ma własne terminale i układ, ale edytuje te same pliki co pozostałe workspaces w tym folderze.',
  'workspace.previewDescription': 'Osobne terminale i układ · współdzielone pliki projektu',
  'workspace.createAction': 'Dodaj workspace',
  'workspace.creating': 'Dodawanie…',
  'worktree.createTitle': 'Nowy workspace z gałęzią',
  'worktree.createDescription':
    'Pracuj w izolowanym Git worktree bez przełączania gałęzi w głównym folderze.',
  'worktree.branchSource': 'Źródło gałęzi',
  'worktree.newBranchDescription': 'Utworzy gałąź z bieżącego HEAD.',
  'worktree.existingBranchDescription': 'Pobierze lokalną gałąź, która nie jest używana.',
  'worktree.newBranchName': 'Nazwa nowej gałęzi',
  'worktree.existingBranchName': 'Gałąź',
  'worktree.newBranchHint': 'Nowa gałąź rozpocznie się od bieżącego HEAD repozytorium.',
  'worktree.existingBranchHint': 'Gałęzie otwarte w innym worktree są niedostępne.',
  'worktree.loadingBranches': 'Ładowanie gałęzi…',
  'worktree.noBranchesAvailable': 'Brak dostępnych gałęzi',
  'worktree.branchInUse': 'w użyciu',
  'worktree.branchLoadFailed': 'Nie udało się załadować gałęzi.',
  'worktree.workspaceName': 'Nazwa workspace',
  'worktree.workspaceNamePlaceholder': 'Taka sama jak nazwa gałęzi',
  'worktree.workspaceNameHint':
    'Ta nazwa pojawi się na pasku bocznym. Pozostaw puste, aby użyć nazwy gałęzi.',
  'worktree.optional': 'Opcjonalne',
  'worktree.isolated': 'Izolowany',
  'worktree.createAction': 'Utwórz workspace',
  'worktree.creating': 'Tworzenie…',
  'worktree.removeDirtyBody':
    'Ten workspace ma niezapisane zmiany, które zostaną utracone. Usunąć mimo to?',
  'worktree.removeRunningBody':
    'Ten workspace ma działające terminale. Zatrzymać je i usunąć?',
  'worktree.revealInFinder': 'Pokaż worktree w menedżerze plików',
  'branch.switch': 'Przełącz gałąź',
  'branch.localGroup': 'Lokalne',
  'branch.search': 'Szukaj gałęzi…',
  'branch.current': 'bieżąca',
  'branch.inUse': 'w użyciu',
  'branch.none': 'Brak gałęzi',
  'branch.loading': 'Ładowanie gałęzi…',
  'branch.switching': 'Przełączanie…',
  'branch.creating': 'Tworzenie…',
  'branch.createFrom': 'Utwórz „{branch}” z {from}',
  'branch.dirtyBody': 'Niezapisane zmiany blokują przełączenie na „{branch}”. Schować je (stash) i przełączyć?',
  'branch.stashAndSwitch': 'Schowaj i przełącz',
  'branch.stashedNote': 'Twoje zmiany zostały schowane. Przywróć je później poleceniem: git stash pop',
  'branch.switchFailed': 'Nie udało się przełączyć gałęzi: {message}',

  'settings.integrations': 'Integracje',
  'integrations.desc':
    'Połącz konto GitHub, GitLab lub Gitea, aby przeglądać i klonować jego repozytoria z paska bocznego.',
  'integrations.empty': 'Brak integracji — dodaj poniżej.',
  'integrations.add': 'Dodaj integrację',
  'integrations.edit': 'Edytuj',
  'integrations.remove': 'Usuń',
  'integrations.provider': 'Dostawca',
  'integrations.name': 'Nazwa',
  'integrations.namePlaceholder': 'np. Firmowa Gitea',
  'integrations.url': 'Adres URL serwera',
  'integrations.optional': 'opcjonalne',
  'integrations.token': 'Token dostępu',
  'integrations.tokenPlaceholder': 'Osobisty token dostępu',
  'integrations.test': 'Testuj połączenie',
  'integrations.testing': 'Testowanie…',
  'integrations.testOk': 'Połączono jako {user}',
  'integrations.testOkNoUser': 'Połączenie udane',
  'integrations.cancel': 'Anuluj',
  'integrations.save': 'Zapisz',
  'integrations.errMissingToken': 'Wprowadź token dostępu.',
  'integrations.errMissingUrl': 'Wprowadź adres URL serwera.',
  'integrations.errUnauthorized': 'Nieprawidłowy token lub niewystarczające uprawnienia.',
  'integrations.errRefused': 'Połączenie odrzucone — czy serwer działa i adres URL jest poprawny?',
  'integrations.errNotFound': 'Nie znaleziono serwera — sprawdź adres URL.',
  'integrations.errTimeout': 'Serwer nie odpowiedział na czas.',
  'integrations.errNetwork': 'Nie można połączyć się z serwerem.',
  'integrations.errUnknown': 'Nie znaleziono integracji.',
  'integrations.errInvalidRepo': 'To repozytorium nie ma adresu klonowania.',
  'integrations.errDestExists': 'Folder docelowy już istnieje i nie jest pusty.',
  'integrations.errGitMissing': 'Git nie jest zainstalowany lub nie ma go w PATH.',
  'integrations.errHttp': 'Serwer zwrócił błąd (HTTP {status}).',
  'clone.integration': 'Integracja',
  'clone.search': 'Szukaj repozytoriów…',
  'clone.loading': 'Ładowanie repozytoriów…',
  'clone.noRepos': 'Nie znaleziono repozytoriów.',
  'clone.private': 'Prywatne',
  'clone.cloning': 'Klonowanie…',
  'openProject.title': 'Otwórz / sklonuj projekt',
  'openProject.thisPc': 'Ten komputer',
  'openProject.fromGit': 'Z Gita',
  'openProject.remoteSsh': 'Remote SSH',
  'openProject.thisPcHint': 'Otwórz istniejący folder projektu z tego komputera.',
  'openProject.chooseFolder': 'Wybierz folder…',
  'openProject.noIntegrations': 'Brak integracji git. Dodaj jedną, aby klonować repozytoria.',
  'openProject.addIntegration': 'Dodaj integrację',
  'remote.hint':
    'Użyj istniejącego aliasu SSH albo user@host. Presety terminala uruchomią się na serwerze w wybranej ścieżce.',
  'remote.host': 'Host SSH',
  'remote.hostPlaceholder': 'np. prod lub user@example.com',
  'remote.path': 'Ścieżka remote',
  'remote.pathPlaceholder': '/home/user/project',
  'remote.name': 'Nazwa wyświetlana',
  'remote.namePlaceholder': 'Opcjonalne',
  'remote.test': 'Testuj',
  'remote.testOk': 'Folder remote jest dostępny.',
  'remote.add': 'Dodaj remote workspace',
  'remote.localOnlyPanel':
    'Remote workspace w tej wersji obsługuje terminale. Pliki, Git, historia i zadania pozostają dostępne tylko dla projektów lokalnych.',

  'common.close': 'Zamknij',
  'common.dismiss': 'Zamknij powiadomienie',
  'titlebar.gitLoading': 'Git…',
  'titlebar.initGit': 'Zainicjuj git',
  'titlebar.gitUnavailable': 'Git niedostępny',
  'titlebar.initGitTitle': 'Zainicjuj repozytorium Git w tym folderze',
  'terminal.statusWorking': 'Odbieranie danych terminala…',
  'terminal.statusFinished': 'Terminal wymaga uwagi',
  'terminal.statusError': 'Proces zakończył się błędem',
  'terminal.exitedChip': 'Zakończony (kod {code})',
  'terminal.notRunningChip': 'Proces już nie działa',
  'terminal.closeRunningConfirm': 'Zatrzymać „{label}”? Działający proces zostanie zabity.',
  'tab.closeRunningConfirm': 'Zamknąć „{name}”? Zostanie zabitych {count} działających terminali.',
  'preview.unsavedTitle': 'Niezapisane zmiany',
  'preview.unsavedConfirm': 'Odrzucić niezapisane zmiany w tym pliku?',
  'preview.discard': 'Odrzuć',
  'preview.changedOnDiskTitle': 'Plik zmienił się na dysku',
  'preview.changedOnDiskConfirm':
    'Ten plik został zmieniony poza edytorem po jego wczytaniu. Nadpisać te zmiany?',
  'preview.overwrite': 'Nadpisz',
  'launcher.deletePresetConfirm': 'Usunąć układ „{name}”?',
  'changes.stageAllCommit': 'Dodaj wszystko i commituj',
  'changes.commitHint': 'Wpisz komunikat commita, aby aktywować przycisk.',
  'history.loadFailed': 'Nie udało się wczytać historii.',
  'history.justNow': '<1 min',
  'history.loadMore': 'Pokazano {n} commitów — wczytaj więcej',
  'files.loadFailed': 'Nie udało się wczytać plików.',
  'files.clearSearch': 'Wyczyść wyszukiwanie',
  'files.refresh': 'Odśwież pliki',
  'search.noResults': 'Brak wyników',
  'tasks.errWorktreeMissing': 'Nie udało się utworzyć worktree.',
  'tasks.failedToast': 'Zadanie nie powiodło się: {prompt}',
  'tasks.retry': 'Ponów zadanie',
  'tasks.moveUp': 'Przenieś wyżej',
  'tasks.moveDown': 'Przenieś niżej',
  'keyboard.conflict': '{chord} jest już używany przez akcję „{action}”.',
  'keyboard.focusCell': 'Fokus na terminal 1–9',
  'presets.markActive': 'Oznacz {name} jako aktywny',
  'daemons.loading': 'Wczytywanie sesji demona…',
  'launcher.morePresets': '+{n} więcej w ustawieniach presetów',
  'terminal.hiddenSessions': '{n} terminali ukrytych — siatka jest pełna',
  'fileOpener.title': 'Otwieraj pliki w',
  'fileOpener.desc': 'Edytor używany po kliknięciu ścieżki pliku w terminalu z Cmd/Ctrl.',
  'fileOpener.system': 'Domyślna aplikacja systemu',
  'sidebar.searchWorkspaces': 'Szukaj workspace…',
  'sidebar.filterFavorites': 'Ulubione',
  'sidebar.showAll': 'Wszystkie workspace’y',
  'sidebar.recentlyVisited': 'Ostatnio odwiedzane',
  'sidebar.noMatches': 'Brak workspace pasujących do wyszukiwania.',
  'sidebar.clearSearch': 'Wyczyść wyszukiwanie workspace',
  'sidebar.favorite': 'Dodaj do ulubionych',
  'sidebar.unfavorite': 'Usuń z ulubionych',
  'terminal.noWorkspaceTitle': 'Zacznij od projektu',
  'terminal.noWorkspaceDescription': 'Otwórz lokalny folder, sklonuj repozytorium Git albo połącz się przez SSH.',
  'terminal.openLocal': 'Otwórz folder',
  'terminal.cloneGit': 'Klonuj z Git',
  'terminal.connectSsh': 'Połącz przez SSH',
  'clone.retry': 'Spróbuj ponownie',
  'clone.openIntegrationSettings': 'Otwórz ustawienia integracji',
  'rightPanel.tabListLabel': 'Narzędzia projektu',
  'tab.listLabel': 'Karty terminali',
  'footer.tickets': "Tickety resetowania",
  'footer.applyTicket': "Użyj jednego ticketu",
  'footer.applying': "Stosowanie…",
  'footer.ticketsUnknown': "Nie udało się pobrać dostępności ticketów.",
  'footer.noTickets': "Brak dostępnych ticketów.",
  'footer.ticketExpires': "Wygasa",
  'footer.ticketNoExpiry': "Nie podano daty wygaśnięcia",
  'footer.ticketDetailsUnknown': "Daty wygaśnięcia nie są dostępne dla wszystkich ticketów.",
  'footer.resetConfirm': "Użyć ticketu resetowania Codex?",
  'footer.resetWarning': "Zużyjesz jeden dostępny ticket do resetowania limitów tego konta Codex. Profile tego samego konta współdzielą limity. Dostawca wybiera ticket. Kontynuować?",
  'footer.resetDone': "Reset Codex przetworzony. Odświeżanie użycia…",
  'footer.reset_nothingToReset': "Nie ma limitów do zresetowania.",
  'footer.reset_noCredit': "Brak dostępnego ticketu.",
  'footer.reset_accountChanged': "Zalogowane konto się zmieniło. Odśwież dane i potwierdź ponownie.",
  'footer.reset_unavailable': "Nie udało się zweryfikować resetu. Ponów, aby bezpiecznie sprawdzić to samo żądanie.",
  'footer.reset_busy': "Inny reset oczekuje na wynik. Ponów go w pierwotnym profilu.",
}
